...
<script>
import {getPageByStatusExe} from '../../service/decision'
  export default {
    data() {
      return {
    ...
    mounted(){
      ...
      this.$root.$emit.apply(this.$root, ['change-header'].concat(["决策执行清单",true, true]));
      //获取列表
      getPageByStatusExe(this.status).then(value => {
        this.issues = value;
      });
    },
    methods: {
