  ...
  <div class="content">
    ...
    <section class="hy_list_container">
      <div class="hylist_container">
        <ul>
          <li class="hy_li" @click="clickItem(item.id, item.type)" v-for="item in list">
            <section>
...
<script>
  import {getPage4Todo} from '../../service/work'
  export default {
    data() {
      return {
        list : []
    ...
    mounted(){
      this.$root.$emit.apply(this.$root, ['change-header'].concat(["待办事项",true, true]));
      //获取列表
      getPage4Todo().then(value => {
        this.list = value;
      });
    },
    methods: {
      clickItem: function (id, type) {
      	if(type=="issue"){
